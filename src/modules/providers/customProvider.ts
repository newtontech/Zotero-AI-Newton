 ...
 *
 * Implements the LLMProvider interface for custom OpenAI-compatible endpoints.
 * This allows users to use any OpenAI-compatible API (local models, etc.)
 */
...
export class CustomProvider implements LLMProvider {
  ...
  validate(): { valid: boolean; error?: string } {
    if (!this.apiBase) {
      return {
        valid: false,
        error:
          getString("workspace-error-missing-api-base") ||
  ...
  async complete(request: LLMRequest): Promise<LLMResponse> {
    ...
    const response = await this.fetchWithTimeout(
      ...
      120000, // Longer timeout for custom/local endpoints
    );
  ...
  private buildPayload(
    ...
    if (request.structuredOutput && request.jsonSchema) {
  ...
  private parseResponse(data: unknown): LLMResponse {
    ...
    if (message.tool_calls) {
      result.toolCalls = message.tool_calls as any[];
    }
